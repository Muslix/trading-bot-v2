...
class TestAdvancedMetrics:
    ...
    def test_beta_calculation(self, mock_fetch):
        ...
        def mock_fetch_side_effect(symbol, period):
            if symbol == 'BTC':
                return pd.DataFrame({'Close': btc_prices}, 
                                  index=pd.date_range('2023-01-01', periods=100))
            else:
                return pd.DataFrame({'Close': asset_prices}, 
                                  index=pd.date_range('2023-01-01', periods=100))
...
class TestMetricsValidation:
    ...
    def test_metrics_bounds_checking(self, mock_fetch):
        ...
        extreme_prices = [p * (1 + np.random.normal(0, 0.05)) for p in base_prices]  # Add volatility
        mock_data = pd.DataFrame({
