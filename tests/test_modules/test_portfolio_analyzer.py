...
class TestCalculateCryptoMetrics:
    ...
    @pytest.mark.asyncio
    async def test_calculate_metrics_valid_symbol(self):
        """Test Metriken-Berechnung für gültiges Symbol"""
        symbol = "BTC"
        result_symbol, metrics = await calculate_crypto_metrics(symbol)
    ...
    @pytest.mark.asyncio
    async def test_calculate_metrics_reproducible(self):
        """Test dass Ergebnisse für gleiche Symbole reproduzierbar sind"""
        symbol = "ETH"

        result1 = await calculate_crypto_metrics(symbol)
        result2 = await calculate_crypto_metrics(symbol)

        assert result1 == result2

    @pytest.mark.asyncio
    async def test_calculate_metrics_different_symbols(self):
        """Test dass verschiedene Symbole verschiedene Ergebnisse liefern"""
        btc_result = await calculate_crypto_metrics("BTC")
        eth_result = await calculate_crypto_metrics("ETH")
...
class TestPortfolioAnalysis:
    ...
    @pytest.mark.asyncio
    async def test_analyze_portfolio_parallel_structure(self):
        """Test Struktur der parallelen Portfolio-Analyse"""
        symbols = ["BTC", "ETH", "ADA"]

        results = await analyze_crypto_portfolio_parallel(symbols)
    ...
    @pytest.mark.asyncio
    async def test_analyze_portfolio_sequential_structure(self):
        """Test Struktur der sequentiellen Portfolio-Analyse"""
        symbols = ["BTC", "ETH"]

        results = await analyze_crypto_portfolio_sequential(symbols)
    ...
    @pytest.mark.asyncio
    async def test_parallel_vs_sequential_same_results(self):
        """Test dass parallel und sequential gleiche Ergebnisse liefern"""
        symbols = ["BTC", "ETH", "ADA"]

        parallel_results = await analyze_crypto_portfolio_parallel(symbols)
        sequential_results = await analyze_crypto_portfolio_sequential(symbols)

        assert parallel_results == sequential_results

    @patch("src.modules.portfolio_analyzer.Pool")
    @pytest.mark.asyncio
    async def test_parallel_uses_multiprocessing(self, mock_pool):
