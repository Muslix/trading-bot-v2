...
class DuplicatePreventionTest:
    ...
    def cleanup_test_data(self):
        ...
        try:
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM arbitrage_alerts WHERE symbol LIKE ?", (f"{self.test_prefix}%",)
                )
    ...
    def test_different_alert_allowed(self):
        ...
        try:
            ...
            print(
                f"      Unterschiedlicher Alert: ID={id2}, Profit={different_alert['profit_percentage']}%"
            )
            ...
            with db.get_connection() as conn:
                ...
                if duplicates:
                    ...
                    for dup in duplicates:
                        print(
                            f"         {dup['symbol']} ({dup['buy_exchange']}->{dup['sell_exchange']}): {dup['count']}x"
                        )
    ...
    def print_summary(self, passed_tests, total_tests):
        ...
        print(
            f"🔄 Exakte Duplikate verhindert: {'✅' if results['duplicate_prevention_works'] else '❌'}"
        )
        print(f"🎯 Ähnliche Alerts blockiert: {'✅' if results['similar_alert_blocked'] else '❌'}")
        print(
            f"🚀 Unterschiedliche Alerts erlaubt: {'✅' if results['different_alert_allowed'] else '❌'}"
        )
