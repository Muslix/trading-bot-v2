...
class DuplicatePreventionTest:
    ...
    def cleanup_test_data(self):
        ...
        try:
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM arbitrage_alerts WHERE symbol LIKE ?", ("{self.test_prefix}%",)
                )
                deleted = cursor.rowcount
                conn.commit()
                if deleted > 0:
                    print("   🧹 {deleted} Test-Alerts bereinigt")
                return True
    ...
    def test_exact_duplicate_prevention(self):
        ...
        try:
            ...
            # Test-Alert
            test_alert = {
                "symbol": "{self.test_prefix}BTC/USDT",
            ...
            if id1 > 0 and id2 == -1:
                ...
                error = "Duplikate-Vermeidung fehlgeschlagen: ID1={id1}, ID2={id2}"
                self.test_results["errors"].append(error)
                print("   ❌ {error}")
                return False

        except Exception as e:
            error = "Duplikate-Test fehlgeschlagen: {e}"
            self.test_results["errors"].append(error)
            print("   ❌ {error}")
            return False
    ...
    def test_similar_alert_blocking(self):
        ...
        try:
            # Base Alert
            base_alert = {
                "symbol": "{self.test_prefix}ETH/USDT",
                "buy_exchange": "kraken",
            ...
            print("      Base Alert: ID={id1}, Profit={base_alert['profit_percentage']}%")
            ...
            print("      Ähnlicher Alert: ID={id2}, Profit={similar_alert['profit_percentage']}%")
            ...
            if id1 > 0 and id2 == -1:
                ...
                error = "Ähnliche Alert-Blockierung fehlgeschlagen: ID1={id1}, ID2={id2}"
                self.test_results["errors"].append(error)
                print("   ❌ {error}")
                return False

        except Exception as e:
            error = "Ähnliche Alert-Test fehlgeschlagen: {e}"
            self.test_results["errors"].append(error)
            print("   ❌ {error}")
            return False
    ...
    def test_different_alert_allowed(self):
        ...
        try:
            # Base Alert
            base_alert = {
                "symbol": "{self.test_prefix}ADA/USDT",
            ...
            print(
                "      Unterschiedlicher Alert: ID={id2}, Profit={different_alert['profit_percentage']}%"
            )
            ...
            if id1 > 0 and id2 > 0:
                ...
                error = "Unterschiedliche Alert-Erlaubnis fehlgeschlagen: ID1={id1}, ID2={id2}"
                self.test_results["errors"].append(error)
                print("   ❌ {error}")
                return False

        except Exception as e:
            error = "Unterschiedliche Alert-Test fehlgeschlagen: {e}"
            self.test_results["errors"].append(error)
            print("   ❌ {error}")
            return False
    ...
    def test_time_window(self):
        ...
        try:
            # Alert erstellen
            test_alert = {
                "symbol": "{self.test_prefix}DOT/USDT",
                "buy_exchange": "binance",
                "sell_exchange": "coinbase",
                "buy_price": 10.0,
                "sell_price": 10.3,
            ...
            id1 = db.save_arbitrage_alert(test_alert)
            print("      Alert erstellt: ID={id1}")

            # Prüfe, ob Duplikate-Check funktioniert
            is_duplicate = db.check_recent_arbitrage_alert(test_alert, hours=6)
            print("      Duplikate-Check (6h): {is_duplicate}")

            # Prüfe mit sehr kurzem Zeitfenster
            is_duplicate_short = db.check_recent_arbitrage_alert(
                test_alert, hours=0.001
            )  # ~3.6 Sekunden
            print("      Duplikate-Check (0.001h): {is_duplicate_short}")
            ...
            if id1 > 0 and is_duplicate and not is_duplicate_short:
                ...
                error = "Zeitfenster-Test fehlgeschlagen: ID={id1}, dup_6h={is_duplicate}, dup_short={is_duplicate_short}"
                self.test_results["errors"].append(error)
                print("   ❌ {error}")
                return False

        except Exception as e:
            error = "Zeitfenster-Test fehlgeschlagen: {e}"
            self.test_results["errors"].append(error)
            print("   ❌ {error}")
            return False
    ...
    def test_database_consistency(self):
        ...
        try:
            with db.get_connection() as conn:
                ...
                # Zähle Test-Alerts
                cursor.execute(
                    "SELECT COUNT(*) as count FROM arbitrage_alerts WHERE symbol LIKE ?",
                    ("{self.test_prefix}%",),
                )
                test_count = cursor.fetchone()["count"]
                print("      Test-Alerts in DB: {test_count}")
                ...
                if duplicates:
                    ...
                    for dup in duplicates:
                        print(
                            "         {dup['symbol']} ({dup['buy_exchange']}->{dup['sell_exchange']}): {dup['count']}x"
                        )

                    error = "Duplikate in Test-Daten gefunden: {len(duplicates)} Gruppen"
                    self.test_results["errors"].append(error)
                    return False
                else:
            ...
            error = "Konsistenz-Test fehlgeschlagen: {e}"
            self.test_results["errors"].append(error)
            print("   ❌ {error}")
            return False
    ...
    def print_summary(self, passed_tests, total_tests):
        ...
        print(
            "🔄 Exakte Duplikate verhindert: {'✅' if results['duplicate_prevention_works'] else '❌'}"
        )
        print("🎯 Ähnliche Alerts blockiert: {'✅' if results['similar_alert_blocked'] else '❌'}")
        print(
            "🚀 Unterschiedliche Alerts erlaubt: {'✅' if results['different_alert_allowed'] else '❌'}"
        )
        print("⏰ Zeitfenster funktioniert: {'✅' if results['time_window_test'] else '❌'}")
        print("💾 DB-Konsistenz: {'✅' if results['database_consistency'] else '❌'}")
