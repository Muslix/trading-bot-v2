...
def display_enhanced_portfolio_results(results: Dict[str, Dict], top_n: int = 15):
    ...
    print("="*90)
    print(f"{'Rank':<4} {'Symbol':<8} {'Sharpe':<8} {'Sortino':<8} {'Return%':<8} {'Vol%':<8} {'MaxDD%':<8} {'Beta':<6} {'Price':<10}")
    print("-"*90)
    
    for i, (symbol, metrics) in enumerate(sorted_cryptos[:top_n], 1):
        data_source = "🔴" if metrics.get('data_source') == 'simulated' else "🟢"
        print(f"{i:3d}. {symbol:<8} {metrics['sharpe_ratio']:<8} {metrics['sortino_ratio']:<8} "
...
async def main():
    ...
    try:
        ...
        if top_cryptos:
            ...
            print(f"   Beste Kryptowährung: {best_crypto[0]} (Sharpe: {best_crypto[1]['sharpe_ratio']:.4f})")
