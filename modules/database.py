...
class CryptoDatabaseManager:
    ...
    def check_recent_arbitrage_alert(self, opportunity: Dict, hours: int = 1) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            if exists:
                self.logger.debug(f"🔄 Ähnlicher Alert bereits vorhanden für {opportunity.get('symbol', 'UNKNOWN')} ({opportunity.get('profit_percentage', 0):.1f}%)")
    ...
    def save_arbitrage_alert(self, opportunity: Dict) -> int:
        ...
        with self.get_connection() as conn:
            ...
            self.logger.info(f"💾 Arbitrage Alert gespeichert: {alert_id} - {opportunity.get('symbol', 'UNKNOWN')} ({opportunity.get('profit_percentage', 0):.1f}%)")
            return alert_id
    ...
    def save_price_data(self, symbol: str, exchange: str, price: float, 
                       volume: float = 0, bid: float = 0, ask: float = 0):
    ...
    def get_price_history(self, symbol: str, exchange: str = None, 
    ...
    def save_portfolio_snapshot(self, top_performers: List[Tuple], 
        ...
        with self.get_connection() as conn:
            ...
            performers_json = json.dumps([
                ...
                for symbol, metrics in top_performers
            ])
            
            # Berechne Statistiken
            total_coins = len(top_performers)
            avg_sharpe = sum(metrics['sharpe_ratio'] for _, metrics in top_performers) / total_coins if total_coins > 0 else 0
    ...
    def log_telegram_message(self, chat_id: str, message_type: str, 
                           message_text: str, success: bool = True,
                           telegram_message_id: str = None, error_message: str = None):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (chat_id, message_type, message_text[:500], success, 
                  telegram_message_id, error_message))
